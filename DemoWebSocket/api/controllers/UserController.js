 ...
 */
      ...
      ValidateService(req,res);
        try {
          const data = req.newUser;
             ...
             if(!resUser){
                return res.json({success:false,messages:" create user do not  success"});
             }
            //  sails.socket.b
 ...
 test:(req,res)=>{
     console.log("wellcom to socket")
    if(!req.isSocket){
        return res.badRequest();
       }
      
return res.json({success:true,messages:"test running"});
